...
/*
Vector2D cam_coordinate(size_t x1, size_t y1){
...
Spectrum PathTracer::raytrace_pixel(size_t x, size_t y){
  ...
  double val_x=0; //final value to input to generate ray
  double val_y=0;

  double sum_x=0;   //to take sum when ns_aa!=1
  double sum_y=0;

  if(num_samples==1){
      val_x=((double)x+0.5)/((double)sampleBuffer.w);
      val_y=((double)y+0.5)/((double)sampleBuffer.h);
  }

  else{

      for (int i = 0; i < ns_aa; i++){
      UniformGridSampler2D sam;
      Vector2D s1=sam.get_sample();
      sum_x+=((double)x+s1.x)/((double)sampleBuffer.w);
      sum_y+=((double)y+s1.y)/((double)sampleBuffer.h);
    }
      val_x=sum_x/num_samples;
      val_y=sum_y/num_samples;
  }
...
//   if (num_samples == 1) {
//     Ray r = camera->generate_ray((((double)x+0.5)/ (double)sampleBuffer.w), (((double)y+0.5)/(double)sampleBuffer.h)); // replace 1.0 with your values based on x,y,sampleBuffer.w,sampleBuffer.h
...
// /*
  ...
  int index = (int) ((y * sampleBuffer.w) + x);
    ...
    Ray r = camera->generate_ray(val_x, val_y); // replace 1.0 with your values based on x,y,sampleBuffer.w,sampleBuffer.h
    r.depth = max_ray_depth;
    output = trace_ray(r, true);
    sampleCountBuffer[index] = num_samples;
