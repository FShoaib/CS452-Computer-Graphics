...
namespace CGL {
  ...
  void MeshEdit::init()
  {
    ...
    GLfloat light_position2[] = {.2, 0.25, 0.2, 0.0};
    GLfloat light_diffuse2[] = {0.0, 0.0, 1.0, 1.0};
    GLfloat light_ambient2[] = {.0, .0, .2, 1.0};
    // Enable a single OpenGL light.
    glLightfv(GL_LIGHT0+1, GL_DIFFUSE, light_diffuse2);
    glLightfv(GL_LIGHT0+1, GL_AMBIENT, light_ambient2);
    glLightfv(GL_LIGHT0+1, GL_POSITION, light_position2);
  ...
  void MeshEdit::update_camera()
  {
    ...
    void MeshEdit::renderlights()
    {
      ...
      //   theta = 0;
      //
      // float x = cos(theta*PI/180);
      // float y = sin(theta*PI/180);
      // // Infinite light location.
      // // Note: Viewer is in the positive z direction,
      // // so we make that component significant.
      // GLfloat light_position[] = {x, y, 1.0, 1.0};
      //
      // glPointSize(20.0f);
      // glBegin(GL_POINTS);
      // glVertex3f(x,y, 1.0);
    ...
    void MeshEdit::draw_controlPoints(){
      ...
      for(int i = 0; i < len; i++)
      {
        ...
        if(instance -> type == POLYMESH){

          glPointSize(20.0f);
          // Draw control points
          glBegin(GL_POINTS);
          for (int k = 0; k < node.controlPoints.size(); k++)
          {
            for (int i = 0; i < node.controlPoints[k].size(); i++)
            {
              for (int j = 0; j < node.controlPoints[k][i].size(); j++)
              {
                if (selected.x == k && selected.y == i && selected.z == j)
                  glColor3f(0.0, 1.0, 0.0);
                else
                  glColor3f(1.0, 1.0, 1.0);
                Vector3D pt = node.controlPoints[k][i][j];
          ...
          {
            glBegin(GL_LINES);
            glColor3f(colorpalette[2*k][0],colorpalette[2*k][1],colorpalette[2*k][2]);
            for (int i = 0; i < node.controlPoints[k].size()-1; i++)
            {
              for (int j = 0; j < node.controlPoints[k][i].size()-1; j++)
              {
                Vector3D pt1 = node.controlPoints[k][i][j];
                Vector3D pt2 = node.controlPoints[k][i+1][j];
                Vector3D pt3 = node.controlPoints[k][i+1][j+1];
                Vector3D pt4 = node.controlPoints[k][i][j+1];
                Vector3D pt5 = node.controlPoints[k][i][j];
    ...
    void MeshEdit::resize( size_t w, size_t h)
    {
      ...
      void MeshEdit::cursor_event( float x, float y, unsigned char keys )
      {
        ...
        {
          x *= screen_w / (float)screen_h;
        }
        else
        {
          y *= screen_h / (float)screen_w;
        }
          ...
          if(instance -> type == POLYMESH){
            if (keys == 4)
            {
              if (selected.x >= 0)
              {
                float dx = x - xp;
                float dy = y - yp;
                node.controlPoints[selected.x][selected.y][selected.z] += Vector3D(dx, dy,0);
              }
              else
              {
                Vector2D xy(x,y);
                ...
                for (int k = 0; k < node.controlPoints.size(); k++)
                {
                  for (int i = 0; i < node.controlPoints[k].size(); i++)
                  {
                    for (int j = 0; j < node.controlPoints[k][i].size(); j++)
                    {
                      Vector2D cpxy(node.controlPoints[k][i][j].x, node.controlPoints[k][i][j].y);
                      cout<<xy.x<<","<<xy.y<<" | "<<cpxy.x<<","<<cpxy.y<<endl;
                      if ((cpxy - xy).norm() < thresh)
                      {
                        selected = Vector3D(k,i,j);
                        cout<<"s"<<endl;
                      }
                    }
                  }
