...
//helper function to check if point inside triangle
bool TriTest(float x1, float y1, float x2, float y2, float x3, float y3, float x4, float y4){

  bool l1,l2,l3;
  l1= -(x1 - x2) * (y3 - y2) + (x3 - x2) * (y1 - y2)>=0;
  l2= -(x1 - x3) * (y4 - y3) + (x4 - x3) * (y1 - y3)>=0;
  l3= -(x1 - x4) * (y2 - y4) + (x2 - x4) * (y1 - y4)>=0;
...
void DrawRend::rasterize_triangle( float x0, float y0,
  ...
  //calcuting min, max x,y and round to closest to optimise for better performance
  int min_x = floor(min({x0, x1, x2}));
  int max_x = ceil(max({x0, x1, x2}));
  int min_y = floor(min({y0, y1, y2}));
  int max_y = ceil(max({y0, y1, y2}));

  float sub_plength = 1 / sqrt(sample_rate);
  ...
  sp.psm = psm;
    ...
    //supersampling
       //sub x and y
        for (int sub_x = 0; sub_x <sqrt(sample_rate); ++sub_x) {
          for (int sub_y = 0; sub_y < sqrt(sample_rate); ++sub_y) {

           curr_x = i + sub_x * sub_plength + (sub_plength/2.0);
            curr_y = j + sub_y * sub_plength+ (sub_plength/2.0);

          //part1
         //x = i + 0.5;
          //y = j + 0.5;

            //helper function point-in-triangle test
            if (TriTest(curr_x, curr_y, x0, y0, x1, y1, x2, y2)){
              
              if(tri!=NULL){
            
              //compute barycentric coordinates

              samplebuffer[j][i].fill_color(sub_x,sub_y,tri->color(x0, y0, x1, y1, x2, y2, curr_x, curr_y, sp));
                }


            else{

              //samplebuffer[i][j].fill_pixel(color); part1

              samplebuffer[j][i].fill_color(sub_x,sub_y,color);
                }
  ...
  //              SampleParams sp = SampleParams();ß
  //              sp.psm = psm;
  //              samplebuffer[row][column].fill_color(row, column, tri->color(x0,y0,x1,y1,x2,y2,row,column,sp));

float gradient_inv(float x1, float x2,float y1,float y2){

  float m=0;

  if(y2-y1==0 ||y1-y2==0 || x1-x2==0 ||x2-x1==0){
...
Vector2D intersection(float x1, float x2, float sx3,float sx4,float y1, float y2, float sy3, float sy4){

Vector2D intersect;

intersect.x=(((x1*y2)-(y1*x2)*(sx3-sx4))-((x1-x2)*(sx3*sy4-sy3*sx4)))/((x1-x2)*(sy3-sy4)-(y1-y2)*(sx3-sx4));
intersect.y=(((x1*y2)-(y1*x2)*(sy3-sy4))-((y1-y2)*(sx3*sy4-sy3*sx4)))/((x1-x2)*(sy3-sy4)-(y1-y2)*(sx3-sx4));
...
void DrawRend::rasterize_scanline( float x0, float y0,
  ...
  int min_x = floor(min({x0, x11, x22}));
  int max_x = ceil(max({x0, x11, x22}));
  int min_y = floor(min({y0, y1, y2}));
  int max_y = ceil(max({y0, y1, y2}));
...
float m_invl12=gradient_inv(x0,x11,y0,y1);
float m_invl23=gradient_inv(x11,x22,y1,y2);
float m_invl13=gradient_inv(x0,x22,y0,y2);


vector<float> E12={min({y0,y1}), max(y0,y1),min(x0,x11),m_invl12};
vector<float> E23={min({y1,y2}), max(y1,y2),min(x11,x22),m_invl23};
vector<float> E13={min({y0,y2}), max(y0,y2),min(x0,x22),m_invl13};


edgeTable={E12,E23,E13};

/*
  edgeTable.push_back(E12);
  edgeTable.push_back(E23);
  edgeTable.push_back(E13);
  */

sort(edgeTable.begin(),edgeTable.end(),first_col());

int size_ET = edgeTable.size();
int size_AT= activeEdgeTable.size();
int ai=0; //active table row number

  //activeEdgeTable.push_back(edgeTable[size_ET-1]);

  while(!activeEdgeTable.empty() && !edgeTable.empty()){

 for (int y = min_y; y < max_y; ++y){
    //for (int i = 0; i < size1; ++i) 

      for (int j = 0; j < size_AT; ++j){

      sort(activeEdgeTable.begin(),activeEdgeTable.end(),third_col());
      samplebuffer[activeEdgeTable[j][2]][y].fill_pixel(color);
  
        if(y==edgeTable[j][0]){

          activeEdgeTable[ai][0]=y;
          activeEdgeTable[ai][1]=edgeTable[j][1]; 
          activeEdgeTable[ai][2]=edgeTable[j][2];
          activeEdgeTable[ai][3]=edgeTable[j][3];

          ai++;
          //samplebuffer[y][activeEdgeTable[j][2]].fill_color(activeEdgeTable[j][2],activeEdgeTable[j][0],color);

        }

         if(y==edgeTable[j][1]){
          activeEdgeTable.erase(activeEdgeTable.begin()+ai);
        }

        activeEdgeTable[j][2]+=gradient_inv(edgeTable[j][2],edgeTable[j+1][2],edgeTable[j][0],edgeTable[j+1][0]);
