...
class DrawRend : public Renderer {
 public:
  ...
  bool operator()(const vector<float>& E12, const vector<float>& E23) const{
    return E12[0]>E23[0];}
};
...
private:
  ...
  struct SampleBuffer {
    ...
    Color get_pixel_color() {
      ...
      int t_pixels = samples_per_side * samples_per_side; //calculating total pixels

      for (int i = 0; i < samples_per_side; i++) {
        for (int j = 0; j < samples_per_side; j++) {

          average += Color(sub_pixels[i][j].data()) * (1.0 / t_pixels);
        }
      }
        return average;
    }
