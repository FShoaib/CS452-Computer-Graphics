...
Spectrum PathTracer::estimate_direct_lighting(const Ray& r, const Intersection& isect) {
  ...
  // int size_lights=scene->lights.size();

  for(SceneLight* slight: scene->lights){
  
   if(slight->is_delta_light()){
      nsam = 1;
   }
   ...
   for(int j = 0; j<nsam ;j++){
      ...
      Spectrum samples = slight->sample_L(hit_p, &wsample, &dist, &pd);
      w_in = w2o * wsample;

      if(w_in.z<0){
          continue;
      }
      Vector3D shadow_ori = hit_p + (EPS_D * wsample);   
      Ray Shadow(shadow_ori,wsample,depth);
      Shadow.max_t = dist;
      //Shadow.o = hit_p + (EPS_D * wsample);    

      if(!bvh->intersect(Shadow)){
         L_samp += isect.bsdf->f(w_out,w_in) * cos_theta(w_in) * samples/pd;
      }
   }
      L_out += L_samp/nsam;
   }   
...
Spectrum PathTracer::estimate_indirect_lighting(const Ray& r, const Intersection& isect) {
  ...
  Spectrum sbsdf = isect.bsdf->sample_f(w_out,&w_in,&pdf);

  float p = (sbsdf.illum() * 20) + 0.05;
  p = clamp(p,0.0,1.0);
  bool prob = coin_flip(p);   //1 if not terminating

   if(prob){
   //if not terminating
      Vector3D dir = o2w * w_in;
      Vector3D org = hit_p + (EPS_D * dir);
   
      int depth = r.depth - 1;
      Ray create(org,dir,depth);

      Spectrum rad_in = trace_ray(create,isect.bsdf->is_delta());

      Spectrum rad_out = (rad_in * sbsdf * cos_theta(w_in)) / (pdf * (prob));
