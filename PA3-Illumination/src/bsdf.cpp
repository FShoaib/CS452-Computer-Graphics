...
bool BSDF::refract(const Vector3D& wo, Vector3D* wi, float ior) {
  ...
  if(wo.z >= 0 ){
    ...
    float n = 1/ior; //when entering
    float check_tir = 1 - ( (n * n) * (1 - (wo.z * wo.z) ) );
    ...
    if (check_tir < 0){
      return false;
       ...
       // *wi = Vector3D( (-1) * n * wo.x, (-1) * n * wo.y, (-1) * sqrt(1 - (n * n) * (1 - (wo.z * wo.z) ) ) );
      *wi = Vector3D(- 1.0 * n * wo.x, - 1.0*  n * wo.y, - sqrt(check_tir));
      return true;
    }  
  }
...
//exiting
    ...
    float n = ior;
    float check_n = 1 - (n * n) * (1 - wo.z * wo.z );
      ...
      if (check_n < 0){
        ...
        // *wi = Vector3D( (-1) * n * wo.x, (-1) * n * wo.y, sqrt(1 - (n * n)) * (1 - (wo.z * wo.z) ) );
        
            // *wi = Vector3D( (-1) * n * wo.x, (-1) * n * wo.y, sqrt( 1 - (n * n) * (1 - (wo.z * wo.z) ) ) );
              *wi = Vector3D(- 1.0 * n * wo.x, - 1.0*  n * wo.y, sqrt(check_n));
            return true;
      }     
    } 
}
